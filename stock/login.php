...
// }
    ...
    $_SESSION["username"] = $_COOKIE['LMB_CONNECT_USERNAME'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($username_err) && empty($password_err)) {
        ...
        $sql = "SELECT id, username, password, TOTP FROM users WHERE username = ?";
                ...
                if (mysqli_stmt_num_rows($stmt) == 1) {
                    ...
                    if (mysqli_stmt_fetch($stmt)) {
                        if (password_verify($password, $hashed_password)) {
                            ...
                            if ($tfa->verifyCode($secret, $_POST["totp"]) === true) {
                                session_start();
                                ...
                                setcookie('LMB_CONNECT_USERNAME', $username, time() + 1*9*3600, '/', 'lmbruleurs.fr', true, true);
...
<head>
    <meta charset="UTF-8">
    <title>STOCK.LMBRULEURS.FR</title>
...
<body>
    <div class="wrapper" style="margin: 0 auto; text-align: center;">
        <h2><a style="color:inherit; text-decoration: inherit " href="https://lmbruleurs.fr" class="fullwidth">LM
                BRULEURS</a></br>ALPES BRULEURS SERVICES</br>STOCK</h2>
        <h2>Connexion</h2>
        <p>Veuillez remplir vos identifiants pour vous connecter.</p>
        </br>
        <a href="https://lmbruleurs.fr/2fa/" target="_blank">https://lmbruleurs.fr/2fa/</a>
        </br> </br>
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <div class="form-group <?php echo (!empty($TOTP_err)) ? 'has-error' : ''; ?>">
                ...
                <input type="password" name="totp" class="form-control">
